...
import { FireblocksHederaClient, signMultipleMessages } from './FireblocksHederaClient';
 ...
 */
export class FireblocksHederaSigner implements Signer, FireblocksHederaSignerAdditionalFunctionality {
	...
	private cachedTransactionPayloadSignatures: { [bodyBytes: string]: Uint8Array } = {};
	...
	async getPublicKey(): Promise<PublicKey> {
		...
		this.publicKey = PublicKey.fromBytesED25519(Buffer.from(publicKey.replace('0x', ''), 'hex'));
		return this.publicKey;
	}
	...
	public async getAccountBalance(): Promise<AccountBalance> {
		return await new AccountBalanceQuery().setAccountId(this.accountId).execute(this.client);
	}

	public async getAccountInfo(): Promise<AccountInfo> {
		return await new AccountInfoQuery().setAccountId(this.accountId).execute(this.client);
	}

	public async getAccountRecords(): Promise<TransactionRecord[]> {
		return await new AccountRecordsQuery().setAccountId(this.accountId).execute(this.client);
	}

	public async sign(messages: Uint8Array[]): Promise<SignerSignature[]> {
		const publicKey: PublicKey = PublicKey.fromBytesED25519(this.vaultAccountPublicKey);
	...
	public async signTransaction<T extends Transaction>(transaction: T): Promise<T> {
		...
		return await transaction.signWith(
			...
			(async (message: Uint8Array): Promise<Uint8Array> => {
				return (
					await signMultipleMessages([message], this.vaultAccountId, this.fireblocksSDK, this.assetId, this.cachedTransactionPayloadSignatures)
				)[0];
			}).bind(this)
		);
	}
